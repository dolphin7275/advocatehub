...
function LawyerReviewSection() {
  ...
  useEffect(() => {
    ...
    const fetchReviews = async () => {
      try {
        const res = await api.get(`/api/lawyers/${lawyerId}/reviews/`);
        setReviews(res.data);
      } catch (err) {
        console.error(err);
  ...
  const handleSubmit = async (e) => {
    ...
    setSubmitting(true);
    try {
      const res = await api.post(`/api/lawyers/${lawyerId}/reviews/`, {...formData, lawyer_id : lawyerId,});
      ...
      {reviews.map((review) => (
        ...
        >
          <div className="flex justify-between items-center">
            <p className="text-lg font-semibold text-gray-800">🧑 {review.user.username}</p>
